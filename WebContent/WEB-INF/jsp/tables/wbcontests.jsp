...
<display:table id="contest" name="contests" class="volume" requestURI=""
	...
	<display:column titleKey="tablehdr.timeremaining">
		<div class="countdown"
			start-data-time="${contest.startDate.time + mapsites.get(contest.sid).offset}"
			end-data-time="${contest.endDate.time + mapsites.get(contest.sid).offset}"></div>
	</display:column>
	<display:column titleKey="tablehdr.status">
		<c:choose>
			<c:when test="${contest.startDate.time <= now}">
