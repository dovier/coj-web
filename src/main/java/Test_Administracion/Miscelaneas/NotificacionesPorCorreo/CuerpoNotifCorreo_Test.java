...
public class CuerpoNotifCorreo_Test {
   ...
   private Utiles_TestAdministracionNotificacionesCorreo utiles_testAdministracionNotificacionesCorreo;
    ...
    //Validando cuerpo de notifiaciones por correo
    @Test
    public void validarCuerpoNotificacionesCorreo() throws Exception {

        ArrayList<String> lista1 = utiles_testAdministracionNotificacionesCorreo.getCuerposCorrectosLenguajeProgramacion();
        ArrayList<String> lista2 = utiles_testAdministracionNotificacionesCorreo.getCuerposIncorrectosLenguajeProgramacion();

        String direccionAux="/admin/notify.xhtml";

        for (String a : lista1) {
            r = mockMvc.perform(post(direccionAux)
                    .param("subject", utiles_testAdministracionNotificacionesCorreo.getAsuntoCorrecto())
                    .param("text", a)
            ...
            Assert.assertEquals("Validacion de cuerpo incorrecta", "/admin/index.xhtml",o );
        }


        for (String a : lista2) {
            r = mockMvc.perform(post(direccionAux)
                    .param("subject", utiles_testAdministracionNotificacionesCorreo.getCuerpoCorrecta())
                    .param("text", a)
                    .session((MockHttpSession) result.getRequest().getSession()))
                    .andReturn();
            ...
            Assert.assertEquals("Validacion de cuerpo incorrecta", null,acutal );
