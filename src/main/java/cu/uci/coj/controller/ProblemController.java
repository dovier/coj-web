...
public class ProblemController extends BaseController {
	...
	public String Problem(HttpServletRequest request, Locale locale, Model model, Principal principal, @RequestParam Integer pid) {
		...
		Problem problem = problemDAO.getProblemByCode(
				locale.getLanguage(),
				pid,
				request.isUserInRole(Roles.ROLE_ADMIN) || request.isUserInRole(Roles.ROLE_SUPER_PSETTER)
						|| (problemDAO.bool("is.psetter.problem", uid, pid) && request.isUserInRole(Roles.ROLE_PSETTER)));
		...
		if (request.isUserInRole(Roles.ROLE_USER)) {
			...
			recommendations = recommender.findSimilarProblems(getUsername(principal), problem);
		} else {
	...
	public String problems(Model model, Locale locale, Principal principal, PagingOptions options, @RequestParam(required = false, value = "pattern") String pattern,
		...
		model.addAttribute("classifications", problemDAO.objects("problem.classifications", ProblemClassification.class));
	...
	public String tablesProblems(Model model, Locale locale, Principal principal, PagingOptions options, @RequestParam(required = false, value = "pattern") String pattern,
			@RequestParam(required = false, defaultValue = "0", value = "filterby") Integer filterby,
			@RequestParam(required = false, defaultValue = "-1", value = "classification") Integer idClassification,
			@RequestParam(required = false, defaultValue = "-1", value = "complexity") Integer complexity) {
	...
	public String ProblemPdf(Locale locale, HttpServletRequest request, HttpServletResponse response, OutputStream os, Model model, Principal principal, @RequestParam Integer pid) {
		...
		try {
			...
			response.setHeader("Content-disposition", "inline; filename=" + problem.getPid() + ".pdf");
			FileInputStream in = new FileInputStream(Config.getProperty("pdfs.dir") + "/" + problem.getPid() + ".pdf");
	...
	public String Votes(Locale locale, Principal principal, SecurityContextHolderAwareRequestWrapper requestWrapper, Model model, @RequestParam Integer pid) {
		...
		if (!problemDAO.bool("problem.voted.byuser", uid, problem.getPid())) {
			...
			VotesStats datos = problemDAO.object("problem.load.votes", VotesStats.class, problem.getPid());
	...
	public String VoteProblem(Model model, Principal principal, @RequestParam Integer pid, @RequestParam(required = true, value = "vote1") String vote1,
			@RequestParam(required = true, value = "vote2") String vote2, @RequestParam(required = true, value = "vote3") String vote3, @RequestParam(required = true, value = "vote4") String vote4,
		...
		try {
			...
			double[] V = { Double.parseDouble(vote1), 0, Double.parseDouble(vote2), 0, Double.parseDouble(vote3), 0, Double.parseDouble(vote4), 0, Double.parseDouble(vote5), 0 };
			...
			problemDAO.dml("problem.update.votes", V[0], V[1], V[2], V[3], V[4], V[5], V[6], V[7], V[8], V[9], pid);
