...
public class SubmissionController extends BaseController {
	...
	public String CompileInfo(Principal principal, Model model,
	...
	public void SourceZipDownload(
			...
			Model model,
			@RequestParam(value = "status", required = false, defaultValue = "0") Integer type) {
		try {
			int uid = submissionDAO.integer("select.uid.by.username",
					getUsername(principal));
			...
			response.setHeader("Content-disposition", "inline; filename="
					+ getUsername(principal) + "-" + name + ".zip");
			FileUtils.crearArchivoComprimido(response.getOutputStream(),
					submissionDAO.getSourceCodes(getUsername(principal), type));
			response.getOutputStream().flush();
		} catch (Exception ex) {
	...
	public String SubmitController(
			...
			@ModelAttribute("submit") SubmissionJudge submit,
			BindingResult bindingResult) {

		List<Language> languages = new LinkedList<Language>();
		Integer uid = userDAO.integer("select.uid.by.username",
				getUsername(principal));
		try {
			if (problemDAO.exists(submit.getPid())) {
				languages.addAll(utilDAO.getEnabledLanguagesByProblem(submit
		...
		submit.setHaspriviligeForProblem(requestWrapper
				.isUserInRole(Roles.ROLE_ADMIN)
				|| requestWrapper.isUserInRole(Roles.ROLE_SUPER_PSETTER)
				|| (problemDAO.bool("is.psetter.problem", uid,
						Integer.valueOf(submit.getPid())) && requestWrapper
		...
		if (submit.isHaspriviligeForProblem() && submit.isTest()) {
			...
			try {
				...
				submission.setStatus("Unqualified");
			}
			if (testSubmit != null)
				model.addAttribute("testSubmit", testSubmit.current(submission.getUid()));
			
			model.addAttribute("submit", submit);
			return SubmitController(principal, requestWrapper, model, submit.getPid(), submit.getSid());
		} else {
			int sid = submissionDAO.insertSubmission(iduser,
					getUsername(principal), problem.getPid(), submit.getCode(),
					submit.getLanguageByLid(), locked, null);

			SubmissionJudge submission = new SubmissionJudge(sid, iduser,
					submit.getCode(), problem.getPid(), problem.getTime(),
					problem.getCasetimelimit(), problem.getMemory(),
					submit.getLanguageByLid());
			submission.setSpecialJudge(problem.isSpecial_judge());
