...
public class ContestController extends BaseController {
	...
	public String tablesListVirtuals(Model model, PagingOptions options) {
		...
		IPaginatedList<VirtualContest> contests = contestDAO.loadVirtualContests(options);
	...
	public String deleteVirtual(Model model, @RequestParam Integer vid, @RequestParam("uid") String username) {
	...
	public String manageContest(Model model, Contest contest, BindingResult result) {
	...
	public String createContest(Model model, Contest contest, BindingResult result) {
	...
	public String globalSettings(Model model, Contest contest, BindingResult result) {
	...
	public String contestProblems(Model model, Locale locale, @RequestParam("cid") Integer cid) {
		Contest contest = contestDAO.loadContestForProblems(cid);
		List<Problem> problems = contestDAO.loadContestProblems(contest.getCid());
		...
		if (contest.getStyle() == 4) {
			for (Iterator<Problem> it = problems.iterator(); it.hasNext();) {
				...
				if (levelT.containsKey(problem.getLevel())) {
					...
					Level lv = new Level(problem.getLevel(), new LinkedList<Problem>());
	...
	public String contestProblems(Model model, Locale locale, Contest contest, BindingResult result) {
		...
		if (contest.getProblemids().length != 0) {
			try {
				for (int i = 0; i < contest.getProblemids().length; i++) {
					contestDAO.insertProblemContest(contest.getProblemids()[i], contest.getCid(), contest.getLevels(), i + 1);
				}
			} catch (Exception e) {
			}
		}
	...
	@RequestMapping(produces = "text/plain", value = "/importicpcusers.xhtml", method = RequestMethod.POST, headers = { "Accept=text/plain" })
	public @ResponseBody() String importICPCUsers(Model model, Locale locale, @RequestParam("cid") Integer cid, @RequestParam(value = "warmupCid", required = false) Integer warmupCid,
			@RequestParam("prefix") String prefix, @RequestParam MultipartFile personFile, @RequestParam MultipartFile schoolFile, @RequestParam MultipartFile siteFile,
			@RequestParam MultipartFile teamFile, @RequestParam MultipartFile teamPersonFile) throws IOException {

		List<String> messages = contestService.importICPCUsers(prefix, new String(personFile.getBytes()).split("\r\n"), new String(schoolFile.getBytes(), "UTF-8").split("\r\n"),
	...
	public String contestProblemColors(Model model, Locale locale, @RequestParam("cid") Integer cid) {
		Contest contest = contestDAO.loadContestForProblems(cid);
		List<Problem> problems = contestDAO.loadContestProblems(contest.getCid());
		contest.setProblems(problems);
		model.addAttribute(contest);
	...
	public String baylorxml(Model model, Locale locale, @RequestParam("cid") Integer cid) {
		model.addAttribute("cid", cid);
	...
	public @ResponseBody() byte[] baylorXml(Model model, Locale locale, HttpServletResponse response, MultipartFile xml, @RequestParam("cid") Integer cid) throws IOException {
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(xml.getInputStream(), "UTF-8"));
		StringBuilder builder = new StringBuilder();
		String line = null;
		while ((line = reader.readLine()) != null) {
			...
			if (i < maps.size()) {
				...
				line = line.replace("TotalTime=\"0\"", "TotalTime=\"" + maps.get(i).get("penalty") + "\"");
				line = line.replace("Rank=\"\"", "Rank=\"" + maps.get(i).get("rank") + "\"");
				line = line.replace("ProblemsSolved=\"0\"", "ProblemsSolved=\"" + maps.get(i).get("accepted") + "\"");
				line = line.replace("LastProblemTime=\"0\"", "LastProblemTime=\"" + maps.get(i).get("last_time") + "\"");
				line = line + "\n";
			}
			builder.append(line);
		}

		response.setHeader("Content-Disposition", "attachment; filename=\"" + xml.getOriginalFilename());
		return builder.toString().getBytes("UTF-8");
	}

	@RequestMapping(value = "/contestproblemcolors.xhtml", method = RequestMethod.POST)
	public String contestProblemColors(Model model, Locale locale, String[] colors, Contest contest) {

		int idx = 0;
		List<Problem> problems = contestDAO.loadContestProblems(contest.getCid());
		for (Problem problem : problems) {
			contestDAO.insertProblemColor(problem.getPid(), contest.getCid(), colors[idx++]);
		}
		return "redirect:/admin/contestproblemcolors.xhtml?cid=" + contest.getCid();
	}
	...
	public String contestUsers(Model model, @RequestParam("cid") Integer cid) {
		...
		model.addAttribute("alljudges", userDAO.loadJudgesOffContest(contest.getCid()));
	...
	public String contestAwards(Model model, ContestAwardsFlags contestawardsflags) {
	...
	public String deleteContestUsers(Model model, @RequestParam("cid") Integer cid, @RequestParam(required = false, value = "all") String all,
	...
	public String contestOverview(Model model, Contest contest) {
		contestDAO.dml("update.contest.overview", contest.getOverview(), contest.getCid());
	...
	public String repointContest(Model model, @RequestParam("cid") Integer cid) throws IOException {
		...
		if (contest.getStyle() == 4) {
			...
			for (int i = 0; i < users.size(); i++) {
				int maxLevel = contestDAO.integer("max.level.pass.by.uid", cid, users.get(i).getUid());
				contestDAO.dml("update.user.level", (usersFull.contains(users.get(i).getUid()) || maxLevel > levels) ? levels : (maxLevel + 1), users.get(i).getUid(), cid);
			}
		}
		return "redirect:/contest/cscoreboard.xhtml?cid=" + cid;
	}

	@RequestMapping(value = "/repointuser.xhtml", method = RequestMethod.GET)
	public String repointUserContest(Model model, @RequestParam("cid") Integer cid, @RequestParam("username") String username) throws IOException {
		Contest contest = contestDAO.loadContest(cid);
		Utils.startUserRepoint(new RepointUser(cid, contest.isFrozen(), contestDAO.integer("select.uid.by.username", username)));
		return "redirect:/contest/cscoreboard.xhtml?cid=" + cid;
	}
	...
	public String removeContestProblem(Model model, @RequestParam("cid") Integer cid, @RequestParam("pid") Integer pid) {
	...
	public String rejudgeContest(Model model, @RequestParam("cid") String cid) throws IOException {
		Rejudge rejudge = new Rejudge(cid);
		Contest contest = contestDAO.loadContest(new Integer(cid));
		model.addAttribute("problems", contestDAO.loadContestProblems(new Integer(cid)));
		model.addAttribute(rejudge);
		model.addAttribute(contest);
		return "/admin/contestrejudge";
	}

	@RequestMapping(value = "/contestrejudge.xhtml", method = RequestMethod.POST)
	public String rejudgeContest(Model model, Rejudge rejudge) throws IOException {

		if (!StringUtils.isEmpty(rejudge.getPid())) {
			// eventManager.publish(new Event(EventType.REJUDGE_PROBLEM, "pid:"
			// + rejudge.getPid()));
		}
		if (!StringUtils.isEmpty(rejudge.getSubmitid())) {
			// eventManager.publish(new Event(EventType.REJUDGE_SUBMIT, "sid:" +
			// rejudge.getSid()));
		}
		if (!StringUtils.isEmpty(rejudge.getRg1()) && StringUtils.isEmpty(rejudge.getRg2())) {
			// eventManager.publish(new Event(EventType.REJUDGE_SUBMIT_RANGE,
			// "sid1:" + rejudge.getRg1() + ";sid2:" + rejudge.getRg2()));
		}

		Utils.startRejudge(rejudge);
		return "/admin/contestrejudge";
	}
}
