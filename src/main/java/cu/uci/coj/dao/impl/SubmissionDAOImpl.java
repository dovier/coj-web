...
public class SubmissionDAOImpl extends BaseDAOImpl implements SubmissionDAO {
	...
	public void applyEffects(SubmissionJudge submit,boolean isDisabling) {
		...
		if (submit.isAccepted()) {
			...
			dml("update.user.problem.accepted", submit.isAccepted(), submit.getPid(), submit.getUid());

			dml("update.last.user.accepted.submit", submit.getSid(), submit.getUid());
			...
			if (bool("is.first.accepted", submit.getSid(), submit.getPid(), submit.getUid())) {
				...
				dml("update.user.profile.substract", totbefore, submit.getPid(), submit.getUid(), totbefore);
	...
	public List<SubmissionJudge> submissions(Integer uid) {
		return objects(getSql("xtats.submission.users.all"), SubmissionJudge.class, uid);
	}
	...
	public int countSubmissions(String username, String language, Integer pid, String status) {
		...
		Query query = new Query("submition").where(Where.yes("enabled"), Where.eq("username", username), Where.eq("language", language), Where.eq("status", status), Where.eq("submition.pid", pid));
	...
	public int countSubmissionsAdminRejudge(String username, String language, Integer pid, String status) {
		...
		query.where(Where.yes("submition.rejudge"), Where.eq("username", username), Where.eq("language", language), Where.eq("status", status), Where.eq("submition.pid", pid));
		return integer(query.count(), query.arguments());
	}
	...
	public IPaginatedList<SubmissionJudge> getSubmissions(String username, int found, String language, Integer pid, String status, PagingOptions options, Boolean loggedIn, Boolean admin, String usern) {
		...
		Query query = new Query("submition");
		query.where(Where.eq("username", username), Where.eq("language", language), Where.eq("status", status), Where.eq("submition.pid", pid));

		query.order(Order.desc("submit_id"));
		query.paginate(options, 20);

		List<SubmissionJudge> submissions = objects(query.select("submition.uid", "username", "submition.pid", "submit_id as sid", "submition.time as time_used", "submition.memory as memory_used",
				"submition.fontsize as font", "submition.date,status", "language as lang", "testcase as FirstWaCase", "average_case as avg_time_used"), SubmissionJudge.class, query.arguments());
	...
	public List<SubmissionJudge> getSourceCodes(String username, int type) {
		...
		switch (type) {
		case 1: {
			subs = objects("get.source.code.ac.username", SubmissionJudge.class, username);
			break;
		}
		default: {
			subs = objects("get.source.code.username", SubmissionJudge.class, username);
			break;
		}
		}
		if (subs != null) {
	...
	public IPaginatedList<SubmissionJudge> getSubmissionsAdminRejudge(String username, int found, String language, Integer pid, String status, PagingOptions options) {
		...
		List<SubmissionJudge> submissions = objects(
				query.select("submition.uid,username,submition.pid,submit_id as sid,submition.time as time_used,submition.memory as memory_used,submition.fontsize as font,submition.date,"
						+ "status,language as lang,testcase,average_case as avg_time_used,enabled,rejudge_status"), SubmissionJudge.class, query.arguments());
		for (SubmissionJudge s : submissions) {
			s.setAuthorize(true);
			s.initialize();
		}
	...
	public SubmissionJudge getSourceCode(int submit_id) {
		SubmissionJudge sub = object("get.source.code", SubmissionJudge.class, submit_id);
		sub.initialize();
		return sub;
	}
	...
	@Transactional(readOnly = true)
	private int getMaxSubmitionIDByUsernameAndPcodeInContest(String username, Integer pid) {
	...
	public int insertSubmission(int iduser, String username, int pid, String source, String language, boolean locked, Integer courseId) {
		...
		dml("insert.submission", iduser, pid, source.length(), username, "Judging", language, Boolean.valueOf(locked));
	...
	private Query getSubmissionsContestQuery(String username, String language, Integer pid, String status, Contest contest) {
		Query query = new Query("contest_submition");
		Where where = Where.and(Where.yes("enabled"), Where.no("virtual"), Where.eq("cid", contest.getCid()), Where.eq("username", username), Where.eq("language", language),
				Where.eq("status", status), Where.eq("pid", pid));

		boolean filter_status = (status != null && !status.isEmpty());

		if (filter_status && contest.isFrozen()) {
			Query subQuery = new Query("contest", "enddate - interval '" + contest.getFrtime() + " Minutes'").where(Where.eq("cid", contest.getCid()));
	...
	@Transactional(readOnly = true)
	public IPaginatedList<SubmissionJudge> getContestSubmissions(int found, String username, String language, Integer pid, String status, PagingOptions options, String usern, boolean loggedIn,
		...
		for (SubmissionJudge submission : submissions) {
			...
			if (!submission.getStatus().equals("Accepted") && !submission.getStatus().equals("Compilation Error") && !submission.getStatus().equals("Unqualified")
					&& !submission.getStatus().equals("Judging") && contest.isShow_ontest()) {
				submission.setOntest(true);
			}
			if (contest.isFrozen() && (submission.getDate().getTime() >= (contest.getEnddate().getTime() - 60000 * contest.getFrtime())) && !admin) {
				if (!loggedIn || (contest.isFull_frozen() && submission.getDate().getTime() >= contest.getEnddate().getTime() - 60000 * contest.getDeadtime())
	...
	private Query getSubmissionsVirtualContest(String username, String language, Integer pid, String status, Contest contest, String current_user) {
		Query subQuery = new Query("individual_virtual_contest join contest using(cid)", "initdate + (now() - start_time)").where(Where.eq("vid", contest.getCid()),
				Where.eq("individual_virtual_contest.username", current_user));
		Query query = new Query("contest_submition").where(
				Where.yes("enabled"),
				Where.eq("username", username),
				Where.eq("language", language),
				Where.eq("status", status),
				Where.eq("pid", pid),
				Where.or(Where.eq("cid", contest.getCid()), Where.eq("cid", new Query("individual_virtual_contest", "cid").where(Where.eq("vid", contest.getCid())))),
				Where.le("date", subQuery),
				Where.or(
						Where.no("virtual"),
						Where.and(
								Where.yes("virtual"),
								Where.or(
										Where.eq("username", current_user),
										Where.in(
												username,
												new Query("virtual_contest_guest", "username").where(Where.eq("vid",
														new Query("individual_virtual_contest", "father").where(Where.eq("username", current_user)))))))));
		return query;
	}
	...
	public int countSubmissionsVirtualContest(String username, String language, Integer pid, String status, Contest contest, String current_user) {
		Query query = getSubmissionsVirtualContest(username, language, pid, status, contest, current_user);
		return integer(query.count(), query.arguments());

	}

	@Transactional(readOnly = true)
	public IPaginatedList<SubmissionJudge> getVirtualContestSubmissions(int found, String username, String language, Integer pid, String status, PagingOptions options, boolean is_admin,
			Contest contest, String current_user) {
		IVirtualStatusSolutions rowmapper = new IVirtualStatusSolutions(current_user, is_admin, contest);
	...
	private Query getContestSubmissionsAdminQuery(String username, String language, Integer pid, String status, int cid) {
		return new Query("contest_submition").where(Where.eq("username", username), Where.eq("language", language), Where.eq("status", status), Where.eq("pid", pid), Where.eq("cid", pid));
	}

	@Transactional(readOnly = true)
	public int countSubmissionsContestAdmin(String username, String language, Integer pid, String status, int cid) {
		Query query = getContestSubmissionsAdminQuery(username, language, pid, status, cid);
		return integer(query.count(), query.arguments());
	}
	...
	public SubmissionJudge getContestCompileInfo(int sid, int cid) {
		return object("get.contest.compile.info", SubmissionJudge.class, cid, sid);
	}
	...
	public int insertVirtualSubmission(int iduser, String username, int pid, String source, String language, int cid, boolean virtual, Contest contest) {
		dml(replaceSql("insert.virtual.submission.1", "<vstime>", virtualSubmissionTime(username, contest.getCid())), iduser, pid, username, "Judging", language, cid, virtual);
		int sid = getMaxSubmitionIDByUsernameAndPcodeInContest(username, pid);
	...
	public SubmissionJudge loadContestSubmissionAdmin(int sid) {
		return object("load.contest.submission.admin", SubmissionJudge.class, sid);
	}

	public void updateSubmission(SubmissionJudge submission) {
		dml("update.submission", new Integer(submission.getTimeUsed()), submission.getMemoryUsed(), new Integer(submission.getCode().length()), submission.getStatus(), submission.isEnabled(),
				submission.getSid());
	...
	public void updateContestSubmission(SubmissionJudge submission) {
		dml("update.contest.submission", new Integer(submission.getTimeUsed()), submission.getMemoryUsed(), new Integer(submission.getCode().length()), submission.getStatus(), submission.isEnabled(),
				submission.getSid());
		dml("update.contest.submission.1", submission.getCode(), submission.getSid());
	}
	...
	@Transactional(readOnly = true)
	public IPaginatedList<SubmissionJudge> bestSolutions(Integer pid, int found, PagingOptions options, SecurityContextHolderAwareRequestWrapper Orequest, Problem Oproblem, boolean see) {
		IBestSolutions bsolutions = new IBestSolutions(pid, options.getPage(), Orequest, Oproblem);
		List<SubmissionJudge> submissions = jdbcTemplate.query(getSql("best.solutions"), bsolutions, pid, options.getOffset(20));
		return getPaginatedList(options, submissions, 20, found);
	}
	...
	@Transactional(readOnly = true)
	public IPaginatedList<SubmissionJudge> bestSolutions(Integer pid, int found, PagingOptions options, SecurityContextHolderAwareRequestWrapper Orequest, Problem Oproblem) {
		IBestSolutions bsolutions = new IBestSolutions(pid, options.getPage() - 1, Orequest, Oproblem);
		return getPaginatedList(options, jdbcTemplate.query(getSql("best.solutions.1"), bsolutions, pid, (options.getPage() - 1) * 20), 20, found);
	}
}
