...
}}(this,function(){var c="2.2.0-javascript",a={},d,g=[],f=[],e=0,b=Object.prototype.hasOwnProperty;
...
}}}function k(){return Y(P,a.util.getAbsoluteURL(P.url)).replace(/^http/,"ws")
...
}if(P.enableProtocol&&aC){var aE=a.util.now()-P.ctime;
P.lastTimestamp=Number(P.stime)+Number(aE)
...
p.onerror=function(aF){clearTimeout(P.id);
...
if(P.enableProtocol&&aC){var aF=a.util.now()-P.ctime;
P.lastTimestamp=Number(P.stime)+Number(aF)
}var aB=k(P.url);
...
}if(P.connectTimeout>0){P.id=setTimeout(function(){if(!aC){var aG={code:1002,reason:"",wasClean:false};
aa.onclose(aG);
try{an()
}catch(aH){}return
}},P.connectTimeout)
}aa.onopen=function(aH){z(P);
...
aa.onerror=function(aG){clearTimeout(P.id)
};
aa.onclose=function(aG){clearTimeout(P.id);
...
}else{if(P.reconnect&&X.transport==="websocket"&&aG.code!==1001){an();
...
}}function v(aF,aE){var aD=aE;
if(aF.transport==="polling"){return aD
}if(a.util.trim(aE).length!==0&&aF.enableProtocol&&aF.firstMessage){var aG=aF.trackMessageLength?1:0;
var aC=aE.split(aF.messageDelimiter);
if(aC.length<=aG+1){return aD
}aF.firstMessage=false;
aF.uuid=a.util.trim(aC[aG]);
aF.stime=a.util.trim(aC[aG+1]);
if(aF.transport!=="long-polling"){ap(aF)
}e=aF.uuid;
aD="";
aG=aF.trackMessageLength?3:2;
if(aC.length>aG+1){for(var aB=aG;
aB<aC.length;
aB++){aD+=aC[aB];
if(aB+1!==aC.length){aD+=aF.messageDelimiter
}}}}else{if(aF.enableProtocol&&aF.firstMessage){}else{ap(aF)
}}return aD
...
}if(aC.lastTimestamp!=null){aB+="&X-Cache-Date="+aC.lastTimestamp
}else{aB+="&X-Cache-Date="+0
...
if(aE.lastTimestamp!=null){aD.setRequestHeader("X-Cache-Date",aE.lastTimestamp)
}else{aD.setRequestHeader("X-Cache-Date",0)
...
var aB={connected:false,timeout:60000,method:"POST",url:P.url,contentType:P.contentType,headers:P.headers,reconnect:true,callback:null,data:aD,suspend:false,maxRequest:-1,logLevel:"info",requestCount:0,withCredentials:P.withCredentials,async:P.async,transport:"polling",isOpen:true,attachHeadersAsQueryString:true,enableXDR:P.enableXDR,uuid:P.uuid,dispatchUrl:P.dispatchUrl,enableProtocol:false,messageDelimiter:"|",trackMessageLength:P.trackMessageLength,maxReconnectOnClose:P.maxReconnectOnClose};
...
X.state=aB;
D()
}function ah(aB,aE){if(!aE.readResponsesHeaders){if(!aE.enableProtocol){aE.lastTimestamp=a.util.now();
aE.uuid=K
}}else{try{var aD=aB.getResponseHeader("X-Cache-Date");
if(aD&&aD!=null&&aD.length>0){aE.lastTimestamp=aD.split(" ").pop()
}var aC=aB.getResponseHeader("X-Atmosphere-tracking-id");
if(aC&&aC!=null){aE.uuid=aC.split(" ").pop()
}}catch(aF){}}}function ac(aB){av(aB,P);
av(aB,a.util)
