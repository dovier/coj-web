...
<display:table id="submission" name="submissions" class="volume"
		...
		<display:column titleKey="tablehdr.id" headerClass="headid">
			<c:choose>
				<c:when test="${submission.authorize==true}">
					<a
						href="<c:url value="submission.xhtml?id=${submission.sid}"/>"><c:out
		...
		<display:column titleKey="tablehdr.date"
			headerClass="headdate" ><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${submission.date}" /></display:column>
		...
		<display:column property="pid" titleKey="tablehdr.prob"
			...
			paramProperty="pid" />
		<display:column titleKey="tablehdr.judgment"
			headerClass="headjudgment">
