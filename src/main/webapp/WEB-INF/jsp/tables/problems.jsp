...
<display:table id="problem" name="problems" class="volume" requestURI="" defaultsort="0" defaultorder="ascending" decorator="cu.uci.coj.utils.tabledecorator.problemsTableDecorator">
		<display:column titleKey="tablehdr.id" class="headidp">
			<c:choose>
				<c:when test="${problem.favorite == true}">
                    ${problem.pid} <a href="unmarkfavorite.xhtml?pid=${problem.pid}" title="<spring:message code="titval.unmarkfavorite"/>"><i class="gold fa fa-star"></i></a>
				</c:when>
				<c:otherwise>
		...
		<display:column property="submitions" titleKey="tablehdr.sub" sortable="true" sortProperty="submitions" sortName="total" autolink="true" headerClass="headsub" href="status.xhtml" paramId="pid" paramProperty="pid" />
		<display:column property="ac" titleKey="tablehdr.ac" sortable="true" sortProperty="ac" sortName="acc" headerClass="headacc" href="status.xhtml?status=ac" paramId="pid" paramProperty="pid" />
