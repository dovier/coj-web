...
<div class="postcontent">
	<center>
		<table class="volume" border="1px">
			...
			<tr>
				...
				<td><a
					href="cuseraccount.xhtml?cid=${contest.cid}&uid=${submission.username}">${submission.username}</a></td>
				<td><a
					href="<c:url value="cproblem.xhtml?pid=${submission.pid}&cid=${contest.cid}"/>"><c:out
							value="${submission.problemTitle}" /></a></td>
				<td width="15%"
					class="<c:out value="sub${submission.statusClass}"/>"
					id="<c:out value="${submission.statusId}"/>"
					name="<c:out value="${submission.statusName}"/>"><b><c:out
							value="${submission.status}" /></b></td>
	...
	<div style="clear: both; float: right;">
		<a
			href="downloadsource.xhtml?sid=${submission.sid}&cid=${contest.cid}"><i
			class="fa fa-save"></i>&nbsp;<spring:message code="link.download" /></a>
	</div>
	<div>
		<textarea cols="" rows="" id="code"
			style="height: 410px; width: 100%; text-align: justify;" readonly>${submission.code}</textarea>
	</div>
</div>
