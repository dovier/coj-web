...
public class Contest {
    ...
    private List<User> practice_locked;

    public List<User> getPractice_locked() {
        return practice_locked;
    }

    public void setPractice_locked(List<User> practice_locked) {
        this.practice_locked = practice_locked;
    }
    ...
    /**
     * Determina si la fecha pasada en milisegundos ocurre dentro del tiempo congelado del contest
     * @param millis
     * @return
     */
    public boolean isInFrozen(long millis) {
        return (millis >= this.getEnddate().getTime() - this.frtime * 60000);
    ...
    public void initCurrlang() {
        currlanguages = new int[languages.size()];
        for (int i = 0; i < languages.size(); i++) {
            currlanguages[i] = languages.get(i).getLid();
        }
    }
    ...
    public List<String> getLetters() {
        ...
        for (Iterator<Problem> it = problems.iterator(); it.hasNext();) {
            String string = ((Problem) it.next()).getLetter() + "";
            letters.add(string);
        }
        return letters;
    }

    public String getProblemLetter(int pid) {
        for (Iterator<Problem> it = problems.iterator(); it.hasNext();) {
            Problem problem = it.next();
            if (problem.getPid() == pid) {
                return problem.getLetter() + "";
            }
        }
        return null;
    }
	...
	public boolean mustUnfreeze(Date now) {
		return blocked && unfreeze_auto && now.getTime() > (this.enddate.getTime() + this.unfreeze_time*60000);
	}
