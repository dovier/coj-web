...
public class SubmissionController extends BaseController {
	...
	public String allSubmissions(Model model, Filter filter, PagingOptions options) {
	...
	public String manageSubmission(Model model, @RequestParam("sid") Integer sid) {
		...
		model.addAttribute("planguages", submissionDAO.objects("get.all.languages", Language.class));
	...
	public String manageSubmission(Principal principal, Model model, SubmissionJudge submission, BindingResult result) {
		...
		submissionDAO.dml("insert.log", "editing SubmissionJudge " + submission.getSid(), getUsername(principal));
		return "redirect:/admin/managesubmission.xhtml?sid=" + submission.getSid();
	}

	@RequestMapping(produces = "application/json", value = "/24h/rejudge.json", method = RequestMethod.GET, headers = { "Accept=application/json" })
	public @ResponseBody() boolean rejudgeSubmit(Model model, @RequestParam(value = "sid") int sid) {
	...
	@Deprecated
	private void validateRejudge(String sid1, String sid2, String pid, String status, BindingResult result) {
		boolean has = false;
		if (sid1 != null && sid2 != null && new Integer(sid2) - new Integer(sid1) > 200) {
			has = true;
		}
		if (has) {
	...
	@Deprecated
	@RequestMapping(value = "/rejudging.xhtml", method = RequestMethod.POST)
	public String rejudgeAction(Model model, Principal principal, @RequestParam(required = false, value = "submitid") String submit_Id, @RequestParam(required = false, value = "sid1") String sid1,
			@RequestParam(required = false, value = "sid2") String sid2, @RequestParam(required = false, value = "pid") String pid, @RequestParam(required = false, value = "status") String status)
			throws Exception {
		submissionDAO.dml("insert.log", "Rejudge " + submit_Id + " " + sid1 + " " + sid2 + " " + pid + " " + status, getUsername(principal));

		Rejudge rejudge = new Rejudge(null, submit_Id, sid1, sid2, pid, status);
		Utils.startRejudge(rejudge);
		// achievementDAO.rejudgeSubmits(rejudge);
		return "redirect:/admin/rejudge.xhtml";
	}

	@RequestMapping(value = "/contestsubmission.xhtml", method = RequestMethod.GET)
	public String contestSubmission(Model model, @RequestParam("sid") Integer sid) {
		SubmissionJudge submission = submissionDAO.loadContestSubmissionAdmin(sid);
		List<Status> statuslist = new LinkedList<Status>();
		String[] ar = Config.getProperty("judge.status").split(",");
		for (int i = 0; i < ar.length; i++) {
			String string = ar[i];
	...
	public String contestSubmission(Model model, SubmissionJudge submission, BindingResult result) {
		validator.validate(submission, result);
		if (result.hasErrors()) {
			List<Status> statuslist = new LinkedList<Status>();
			String[] ar = Config.getProperty("judge.status").split(",");
		...
		return "redirect:/admin/contestsubmission.xhtml?sid=" + submission.getSid();
	}
}
